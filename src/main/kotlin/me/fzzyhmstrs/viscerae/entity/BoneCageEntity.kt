...
open class BoneCageEntity(entityType: EntityType<BoneCageEntity>, world: World): Entity(entityType,world), ModifiableEffectEntity {
    ...
    override var entityEffects: AugmentEffect = AugmentEffect().withDamage(2.0F).withDuration(100)
    private var ticksCaging = 0
    ...
    constructor(world: World,x: Double, y: Double, z: Double, yaw: Float, warmup: Int, owner: LivingEntity): this(RegisterEntity.BONE_CAGE_ENTITY,world){
        this.warmup = warmup
        setOwner(owner)
    ...
    override fun tick() {
        ...
        if (world.isClient) {
            if (playingAnimation) {
                ...
                if (ticksLeft == 14) {
                    for (i in 0..11) {
                        ...
                        world.addParticle(ParticleTypes.POOF, d, e + 1.0, f, g, h, j)
                    }
                }
            }
        } else if (--warmup < 0) {
            ...
            if (--ticksLeft < 0) {
                ticksCaging = entityEffects.duration(0)
            }
            if (ticksCaging > 0){
                if (--ticksCaging <=0){
                    this.discard()
    ...
    private fun damage(target: LivingEntity) {
        ...
        if (livingEntity == null) {
            target.damage(DamageSource.GENERIC, entityEffects.damage(0))
    ...
    companion object{
        ...
        fun conjureCage(world: World,user: LivingEntity,
