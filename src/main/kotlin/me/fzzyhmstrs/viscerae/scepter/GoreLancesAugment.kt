...
class GoreLancesAugment(tier: Int, maxLvl: Int, vararg slot: EquipmentSlot): MiscAugment(tier, maxLvl, *slot),
    ...
    override fun effect(
        ...
        val gle = GoreLanceEntity.createGoreLance(world,user, 1.8f,0.75f,user.yaw,effect,level)
        val bl = world.spawnEntity(gle)
        val data = AugmentPersistentEffectData(world, user, BlockPos.ORIGIN, mutableListOf(), level, effect)
    ...
    override fun persistentEffect(data: PersistentEffectHelper.PersistentEffectData) {
        ...
        val gle = GoreLanceEntity.createGoreLance(data.world,data.user, 1.8f,0.75f,data.user.yaw,data.effect,data.level)
        data.world.spawnEntity(gle)
    }
